...
import { 
    ...
    Activity, Binary, AlertTriangle, RefreshCw, X,
    PlayCircle, FileText, Code2, LucideIcon
} from "lucide-react";
...
export default function BridgeProgramPage() {
    ...
    const fetchResources = async (topic: string) => {
        ...
        try {
            ...
            console.error("Neural Resource Link Offline.");
        } finally {
    ...
    if (!program) {
        return (
            <div className="flex flex-col h-screen items-center justify-center bg-slate-50 space-y-6">
                ...
                <h1 className="text-2xl font-black text-slate-900 uppercase">Bridge Node Not Found</h1>
                <Button onClick={() => router.back()}>Institutional Escape</Button>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-slate-50 p-6 lg:p-12 animate-in fade-in duration-700">
                    ...
                    <Card className="col-span-12 lg:col-span-8 p-0 border border-slate-200 overflow-hidden shadow-xl bg-white">
                        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            ...
                            <span className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-[10px] font-black uppercase text-slate-400">
                                {program?.roadmap.length} Nodes Identified
                            </span>
                        </div>
                                ...
                                <div key={week.week} className="relative pl-12">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="text-xl font-black text-slate-950 uppercase ">{week.topic} Phase</p>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Foundational Competency Node</p>
                                            </div>
                                            <Button 
                                                onClick={() => fetchResources(week.topic)}
                                                variant="ghost" 
                                                className="text-blue-600 hover:text-blue-700 hover:bg-blue-50 text-[10px] font-black uppercase tracking-widest"
                                            >
            ...
            {selectedTopic && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    <Card className="w-full max-w-4xl bg-white rounded-3xl overflow-hidden shadow-2xl relative">
                         <Button 
                            variant="ghost" 
                            onClick={() => setSelectedTopic(null)}
                            className="absolute top-6 right-6 h-10 w-10 p-0 rounded-full hover:bg-slate-100 text-slate-400"
                        >
                            <X className="h-5 w-5" />
                        </Button>
                        <div className="p-10">
                            <h2 className="text-2xl font-black text-slate-950 uppercase  mb-8">Learning Hub: {selectedTopic}</h2>
                            ...
                            {loadingResources ? (
                                ...
                                <div className="grid grid-cols-12 gap-8">
                                    <div className="col-span-12 lg:col-span-8 aspect-video bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 relative shadow-2xl">
                                        <iframe 
                                            src={resources?.video} 
                                    ...
                                    <div className="col-span-12 lg:col-span-4 space-y-4">
                                        <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl group hover:border-blue-400 transition-all cursor-pointer">
                                            <div className="flex items-center gap-4 mb-3">
                                                <FileText className="h-5 w-5 text-blue-600" />
                                                <p className="text-[10px] font-black uppercase text-slate-800 tracking-widest">Study Notes (PDF)</p>
                                            </div>
                                            <h4 className="text-sm font-bold text-slate-600">Institutional {selectedTopic} Deep-Dive</h4>
                                        </div>
                                        <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl group hover:border-blue-600 transition-all cursor-pointer">
                                            <div className="flex items-center gap-4 mb-3">
                                                <Code2 className="h-5 w-5 text-blue-600" />
                                                <p className="text-[10px] font-black uppercase text-slate-800 tracking-widest">Practice Lab</p>
                                            </div>
                                            <h4 className="text-sm font-bold text-slate-600">Verified {selectedTopic} Sandbox</h4>
                                        </div>
                                        <div className="p-6 bg-blue-600 rounded-2xl text-white shadow-lg shadow-blue-600/20">
                                            <p className="text-[10px] font-black uppercase tracking-widest mb-2 opacity-60">Validation Status</p>
                                            <p className="text-xs font-bold leading-relaxed">
                                                DNA analysis confirms that watching the foundational session increases competency by <span className="underline decoration-white underline-offset-4">42%</span>.
                                            </p>
                                        </div>
                                    </div>
                                </div>
